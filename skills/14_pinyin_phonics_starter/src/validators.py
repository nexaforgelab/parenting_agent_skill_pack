"""Validation helpers for 拼音启蒙 Agent."""
...
def validate_pinyin_record(record: Dict[str, Any]) -> List[str]:
    errors: List[str] = []

    if not isinstance(record, dict):
        return ["record 必须是字典类型"]

    error = FieldValidator.required(record.get("pinyin"), "pinyin")
    if error:
        errors.append(error)

    score = record.get("pronunciation_score", 0.0)
    error = FieldValidator.range_float(score, 0.0, 10.0, "pronunciation_score")
    if error:
        errors.append(error)

    tone = record.get("tone_level", 1)
    error = FieldValidator.range_int(tone, 1, 4, "tone_level")
    if error:
        errors.append(error)
