...
def validate_schedule_record(record: Dict[str, Any]) -> List[str]:
    """验证单条日程记录
    ...
    timestamp_errors = validate_timestamp(record.get("timestamp"), "日程时间")
    errors.extend([f"日程时间: {e}" for e in timestamp_errors])

    schedule_type_errors = validate_schedule_type(record.get("schedule_type"))
    errors.extend([f"日程类型: {e}" for e in schedule_type_errors])

    title_errors = validate_title(record.get("title"))
    errors.extend([f"标题: {e}" for e in title_errors])

    if "duration_minutes" in record and record["duration_minutes"] is not None:
        duration_errors = validate_duration_minutes(record["duration_minutes"])
        errors.extend([f"时长: {e}" for e in duration_errors])
...
def validate_schedule_type(value: Any) -> List[str]:
    """验证日程类型
    ...
    """
    errors: List[str] = []
    valid_types = {"pickup", "dropoff", "class", "meeting", "checkup", "activity", "meal", "bedtime", "other"}

    if value is None or str(value).strip() == "":
        errors.append("日程类型不能为空")
        return errors

    normalized = str(value).lower().strip()
    if normalized not in valid_types:
        errors.append(f"日程类型必须是以下之一: {', '.join(sorted(valid_types))}")
    return errors


def validate_priority(value: Any) -> List[str]:
    """验证优先级
    ...
    if normalized not in valid_priorities:
        errors.append(f"优先级必须是以下之一: {', '.join(sorted(valid_priorities))}")
    return errors


def validate_title(value: Any, max_length: int = 200) -> List[str]:
    """验证标题字段
    ...
    if len(title) < 1:
        errors.append("标题至少需要1个字符")
    elif len(title) > max_length:
        errors.append(f"标题不能超过{max_length}个字符")

    return errors


def validate_duration_minutes(value: Any, min_val: int = 0, max_val: int = 480) -> List[str]:
    """验证时长
    ...
    try:
        ...
        errors.append("时长必须是整数")

    return errors


def validate_location(value: Any, max_length: int = 300) -> List[str]:
    """验证地点字段
    ...
    if len(location) > max_length:
        errors.append(f"地点不能超过{max_length}个字符")

    return errors


def validate_participant(value: Any, max_length: int = 100) -> List[str]:
    """验证参与者字段
    ...
    if len(participant) > max_length:
        errors.append(f"参与者不能超过{max_length}个字符")

    return errors


def validate_notes(value: Any, max_length: int = 1000) -> List[str]:
    """验证备注字段
...
def validate_payload_with_details(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "raw_records" in payload:
        ...
        for i, record in enumerate(records):
            record_errors = validate_schedule_record(record)
            if record_errors:
                result["is_valid"] = False
...
def batch_validate_records(records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for i, record in enumerate(records):
        errors = validate_schedule_record(record)
        if errors:
