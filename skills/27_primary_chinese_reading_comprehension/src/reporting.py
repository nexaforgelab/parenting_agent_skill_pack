"""Markdown report renderer for 小学语文阅读理解 Agent.
...
class ProgressVisualizer:
    ...
    def generate_progress_summary(progress: Dict[str, Any]) -> List[str]:
        ...
        summary.append(f"答题总数：{progress.get('total_questions', 0)} 道")
...
class SummaryGenerator:
    ...
    def generate_executive_summary(result: Dict[str, Any]) -> str:
        ...
        if progress:
            ...
            lines.append(f"**平均得分**: {progress.get('average_score', 0.0):.1f}/100")
...
def render_report(result: Dict[str, Any]) -> str:
    ...
    if progress:
        ...
        if progress.get("most_practiced_types"):
            lines.append("")
            lines.append(f"**最常练习题型**: {', '.join(progress['most_practiced_types'])}")

        if progress.get("weak_types"):
            lines.append(f"**薄弱题型**: {', '.join(progress['weak_types'])}")
...
def export_report(result: Dict[str, Any], format: str = "markdown") -> str:
    ...
    if format == "json":
        ...
        records = result.get("raw_records", [])
        columns = ["timestamp", "article_title", "question_type", "score", "accuracy_rate"]
