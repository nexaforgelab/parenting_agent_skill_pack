...
SKILL_ID = "preschool_chinese_literacy"
SKILL_NAME = "幼儿识字启蒙 Agent"
...
def run(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    total_chars = progress.get("total_characters", 0)
    mastered_chars = progress.get("mastered_characters", 0)
    progress_stats = {
        "total_characters": total_chars,
        "mastered_characters": mastered_chars,
        "learning_characters": progress.get("learning_characters", 0),
        "average_accuracy": progress.get("average_accuracy", 0.0),
        "mastery_rate": (mastered_chars / total_chars * 100) if total_chars > 0 else 0.0,
        "difficult_characters": progress.get("difficult_characters", []),
    ...
    result: Dict[str, Any] = {
        ...
        "summary": [
            ...
            f"当前识字进度：共学习 {progress.get('total_characters', 0)} 个汉字，掌握 {progress.get('mastered_characters', 0)} 个。",
