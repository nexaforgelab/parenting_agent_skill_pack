...
try:
    ...
    from .planner import (
        ...
        build_known_facts, next_fields, analyze_photo_trends,
        detect_photo_anomalies, generate_personalized_recommendations,
        aggregate_photo_statistics, analyze_category_distribution
    )
...
def run(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    增强版本：包含趋势分析、异常检测、个性化推荐、照片统计等功能
    ...
    if records:
        batch_result = validate_batch_records(records, "照片")
        if batch_result["invalid_count"] > 0:
            print(f"⚠️  警告：{batch_result['invalid_count']} 条照片记录验证失败")
    ...
    result: Dict[str, Any] = {
        ...
        "photo_statistics": stats.to_dict() if stats else None,
...
def calculate_data_quality_score(records: list, stats) -> float:
    ...
    location_ratio = photos_with_location / len(records)
    score += location_ratio * 20

    photos_with_tags = sum(1 for r in records if r.get("tags"))
    tags_ratio = photos_with_tags / len(records)
    score += tags_ratio * 15

    photos_with_desc = sum(1 for r in records if r.get("description"))
    desc_ratio = photos_with_desc / len(records)
    score += desc_ratio * 15
...
def main() -> None:
    """主函数"""
    parser = argparse.ArgumentParser(description="家庭照片整理 Agent")
    ...
    if result.get("metadata"):
        ...
        print(f"\n📊 分析摘要：")
        print(f"  - 总照片数：{metadata['total_records']}")
        print(f"  - 总相册数：{metadata['total_albums']}")
