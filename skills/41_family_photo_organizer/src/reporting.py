"""Markdown report renderer for 家庭照片整理 Agent.
...
def render_report(result: Dict[str, Any]) -> str:
    ...
    if result.get("photo_statistics"):
        lines.append("## H. 照片统计")
        lines.append("")
        lines.extend(render_photo_stats_table(result.get("photo_statistics", {})))
        lines.append("")
...
def render_photo_stats_table(stats: Dict[str, Any]) -> List[str]:
    """渲染照片统计表格
    ...
    metrics = [
        ...
        ("精选照片", stats.get("favorite_count", 0)),
        ("相册数量", stats.get("albums_count", 0)),
        ("有位置标记", stats.get("photos_with_location", 0)),
        ("有标签", stats.get("photos_with_tags", 0)),
        ("月均拍摄", stats.get("average_photos_per_month", 0))
    ]
    ...
    photos_by_year = stats.get("photos_by_year", {})
    if photos_by_year:
        lines.append("**年度分布**:")
        for year, count in sorted(photos_by_year.items(), reverse=True):
            lines.append(f"- {year}年: {count}张")
        lines.append("")
...
def render_category_distribution(distribution: Dict[str, int]) -> List[str]:
    ...
    if len(sorted_dist) > 0:
        ...
        lines.append(f"📊 最多: {top_category} ({top_count}张)")
...
def generate_summary_text(result: Dict[str, Any]) -> str:
    ...
    if result.get("photo_statistics"):
        stats = result.get("photo_statistics", {})
        total = stats.get("total_photos", 0)
        favorites = stats.get("favorite_count", 0)
        lines.append(f"📸 照片统计: 共{total}张，精选{favorites}张")
        lines.append("")
...
def export_to_html(result: Dict[str, Any]) -> str:
    ...
    lines.append("    <title>家庭照片整理报告</title>")
    lines.append("    <style>")
