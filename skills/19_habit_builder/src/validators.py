"""Validation helpers for 幼儿习惯养成 Agent.
...
def validate_habit_record(record: Dict[str, Any]) -> List[str]:
    """验证习惯记录
    ...
    if "quality_score" in record and record["quality_score"] is not None:
        score_errors = validate_quality_score(record["quality_score"])
        errors.extend([f"质量评分: {e}" for e in score_errors])
...
def validate_quality_score(value: Any) -> List[str]:
    ...
    try:
        ...
        if score < 0 or score > 10:
            errors.append("质量评分必须在 0-10 范围内")
    except (ValueError, TypeError):
        errors.append("质量评分必须是数字")
...
def validate_payload_with_details(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "raw_records" in payload:
        ...
        for i, record in enumerate(records):
            record_errors = validate_habit_record(record)
            if record_errors:
                result["is_valid"] = False
...
def batch_validate_records(records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for i, record in enumerate(records):
        errors = validate_habit_record(record)
        if errors:
