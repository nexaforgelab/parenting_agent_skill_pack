...
class SkillInput:
    ...
    preferences: Dict[str, Any] = field(default_factory=list)
...
class HabitDefinition:
    ...
    habit_id: str
    habit_name: str
    category: Union[str, HabitCategory]
    description: str
    ...
    def __post_init__(self):
        ...
        if isinstance(self.category, str):
            try:
                self.category = HabitCategory(self.category)
            except ValueError:
                self.category = HabitCategory.SELF_CARE
    ...
    def validate(self) -> List[str]:
        ...
        if not self.habit_id:
            errors.append("habit_id 不能为空")
        if not self.habit_name:
            errors.append("habit_name 不能为空")
        if self.target_frequency < 1:
            errors.append("target_frequency 必须为正数")
        if self.target_times_per_day < 1:
            errors.append("target_times_per_day 必须为正数")
        return errors
...
class HabitRecord:
    ...
    def validate(self) -> List[str]:
        ...
        if self.quality_score < 0 or self.quality_score > 10:
            errors.append("quality_score 必须在 0-10 范围内")
        return errors
...
class WeeklyStats:
    ...
    habits_broken: List[str] = field(default_factory=list)
    habits_established: List[str] = field(default_factory=list)
    notes: str = ""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "habits_broken": self.habits_broken,
            "habits_established": self.habits_established,
            "notes": self.notes
        }
