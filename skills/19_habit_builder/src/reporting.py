"""Markdown report renderer for 幼儿习惯养成 Agent.
...
def render_weekly_stats_table(stats: Dict[str, Any]) -> List[str]:
    ...
    metrics = [
        ...
        ("平均质量评分", stats.get("average_quality_score", 0)),
        ("连续天数", stats.get("streak_days", 0)),
        ("获得奖励", stats.get("rewards_earned", 0))
    ]
    ...
    habits_established = stats.get("habits_established", [])
    if habits_established:
        lines.append("**已建立习惯**:")
        for habit in habits_established:
...
def export_to_html(result: Dict[str, Any]) -> str:
    ...
    lines.append("    <title>幼儿习惯养成报告</title>")
    lines.append("    <style>")
    lines.append("        body { font-family: 'Microsoft YaHei', Arial, sans-serif; max-width: 900px; margin: 0 auto; padding: 20px; }")
    lines.append("        h1 { color: #2c3e50; border-bottom: 3px solid #e67e22; padding-bottom: 10px; }")
...
def render_progress_summary(progress_score: Dict[str, Any]) -> str:
    ...
    scores = [
        ("完成率", progress_score.get("completion_score", 0)),
        ("质量评分", progress_score.get("quality_score", 0)),
        ("一致性", progress_score.get("consistency_score", 0))
    ]
