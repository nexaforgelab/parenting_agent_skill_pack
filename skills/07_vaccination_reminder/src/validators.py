"""Validation helpers for 疫苗接种提醒 skills.
...
def validate_vaccination_record(record: Dict[str, Any]) -> List[str]:
    """验证疫苗接种记录.
    ...
    required_fields = ["vaccine_id", "vaccine_name", "dose_number", "scheduled_date"]
    for field in required_fields:
        v = Validator(field)
        v.required(record.get(field), f"疫苗记录缺少必填字段: {field}")
        if not v.is_valid():
            errors.extend(v.get_errors())

    if "dose_number" in record:
        v = Validator("dose_number")
        v.type_check(record["dose_number"], int, "剂次必须是整数")
        v.range_check(record["dose_number"], min_val=1, max_val=10)
        if not v.is_valid():
            errors.extend(v.get_errors())

    if "scheduled_date" in record:
        v = Validator("scheduled_date")
        v.pattern_check(
            record["scheduled_date"],
            r"^\d{4}-\d{2}-\d{2}$",
            "计划日期格式必须是 YYYY-MM-DD"
        )
        if not v.is_valid():
            errors.extend(v.get_errors())

    if "actual_date" in record and record["actual_date"]:
        v = Validator("actual_date")
        v.pattern_check(
            record["actual_date"],
            r"^\d{4}-\d{2}-\d{2}$",
            "实际接种日期格式必须是 YYYY-MM-DD"
        )
        if not v.is_valid():
            errors.extend(v.get_errors())
