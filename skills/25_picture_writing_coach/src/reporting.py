"""Markdown report renderer for 小学看图写话 Agent."""
...
def generate_progress_visualization(progress_stats: Dict[str, Any]) -> str:
    ...
    total = progress_stats.get("total_writings", 0)
    avg_words = progress_stats.get("average_word_count", 0)
    avg_expression = progress_stats.get("average_expression_score", 0)
    avg_structure = progress_stats.get("average_structure_score", 0)
    avg_creativity = progress_stats.get("average_creativity_score", 0)

    lines.append("#### 总体统计")
    lines.append(f"- 总写作次数: {total} 次")
    lines.append(f"- 平均字数: {avg_words:.0f} 字")
    lines.append("")
    ...
    max_label_len = max(len(s[0]) for s in scores)
    for label, score in scores:
        padding = " " * (max_label_len - len(label))
        bar = "█" * int(score / 5) + "░" * (20 - int(score / 5))
        lines.append(f"- {label}{padding} [{bar}] {score:.1f}")
    lines.append("")

    weaknesses = progress_stats.get("common_weaknesses", [])
    strengths = progress_stats.get("strong_areas", [])
