...
class LearningRecord:
    ...
    def validate(self) -> List[str]:
        ...
        if self.error_count < 0:
            errors.append("error_count 不能为负数")
        if not 0 <= self.accuracy_rate <= 1:
            errors.append("accuracy_rate 必须在 0-1 之间")
        return errors

    def is_passed(self, threshold: float = 0.8) -> bool:
        """判断是否达标"""
        return self.accuracy_rate >= threshold
...
class ProgressStats:
    ...
    weak_levels: List[str] = field(default_factory=list)
    summary: str = ""
