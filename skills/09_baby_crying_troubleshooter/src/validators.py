"""Validation helpers for 宝宝哭闹原因排查 skills."""
...
class Validator:
    ...
    def enum_check(self, value: Any, allowed_values: List[Any], message: Optional[str] = None) -> "Validator":
        if value is not None and value not in allowed_values:
            self.errors.append(message or f"{self.field_name} 必须是以下值之一")
        return self
...
def validate_crying_record(record: Dict[str, Any]) -> List[str]:
    """验证哭闹记录."""
    errors: List[str] = []

    if "duration_minutes" in record:
        v = Validator("duration_minutes")
        v.type_check(record["duration_minutes"], (int, float))
        v.range_check(record["duration_minutes"], min_val=0, max_val=300)
        if not v.is_valid():
            errors.extend(v.get_errors())
