"""Planning engine for 家庭买菜清单 Agent.
...
def analyze_purchase_trends(records: List[Dict[str, Any]], days: int = 7) -> List[TrendData]:
    """分析采购趋势"""
    trends = []
    if not records:
        return trends
    ...
    for d in sorted_dates:
        ...
        total_amt = sum(r.get("amount", 0) or 0 for r in day_records)
        daily_counts.append(float(count))
        daily_amounts.append(float(total_amt))
        dates.append(d.isoformat())

    if daily_counts:
        count_trend = TrendData(metric_name="每日采购项数", data_points=daily_counts, dates=dates)
        count_trend.calculate_trend()
        trends.append(count_trend)

    if daily_amounts:
        amount_trend = TrendData(metric_name="每日采购金额", data_points=daily_amounts, dates=dates)
    ...
    for cat, count in category_counts.items():
        ...
        if ratio > 0.6 and total > 5:
            alerts.append(Alert(
                alert_type="采购品类单一",
                severity="info",
                message=f"{cat}品类占比超过60%，建议增加其他品类",
                recommendation="考虑添加蔬果、肉类等均衡采购"
            ))
...
def generate_personalized_recommendations(
    ...
    recommendations.append(Recommendation(
        ...
        priority=1,
        title="合理规划采购清单",
        description="根据家庭需求和季节特点优化采购计划",
        action_items=[
            "提前制定一周菜单",
            "按超市区域规划路线",
            "批量购买耐存储食材"
        ],
        rationale="合理的采购计划可以节省时间和金钱",
        expected_outcome="减少冲动消费，降低食品浪费"
    ))
    ...
    if not alerts and not recommendations:
        recommendations.append(Recommendation(
            ...
            priority=3,
            title="保持良好的采购记录习惯",
            description="继续坚持记录采购情况",
            action_items=["记录每次采购的品类和金额", "定期盘点库存", "根据消耗情况调整采购量"],
            rationale="持续的记录有助于优化采购策略",
            expected_outcome="建立科学的采购管理体系"
        ))
...
def aggregate_weekly_stats(records: List[Dict[str, Any]], week_offset: int = 0) -> Optional[WeeklyStats]:
    ...
    total_items = len(week_records)
    total_amt = sum(r.get("amount", 0) or 0 for r in week_records)

    stats = WeeklyStats(
        week_start=week_start.isoformat(),
        week_end=week_end.isoformat(),
        total_schedules=total_items,
        completed_schedules=total_items,
...
def build_known_facts(payload: Dict[str, Any]) -> List[str]:
    ...
    if records:
        trends = analyze_purchase_trends(records, payload.get("history_days", 7))
        if trends:
            for trend in trends:
...
def build_analysis(payload: Dict[str, Any]) -> List[str]:
    ...
    if records:
        trends = analyze_purchase_trends(records, history_days)
...
def build_deliverables(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    deliverables = {
        ...
        "tables": {
            "timeline": ["时间", "品类", "物品", "数量", "金额", "备注"],
            "weekly_review": ["指标", "本周", "上周", "变化", "下一步"]
        },
        "templates": {
            "daily_log": "今天采购了什么？花了多少钱？有没有冲动消费？",
            "handoff_summary": "给家人的采购摘要：已购物品、待购物品、预算情况。"
        }
    }
    records = payload.get("raw_records") or []
    if records:
