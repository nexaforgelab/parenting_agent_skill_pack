"""Markdown report renderer for 专注力训练 Agent.
...
def render_weekly_stats_table(stats: Dict[str, Any]) -> List[str]:
    ...
    metrics = [
        ("总训练次数", stats.get("total_sessions", 0)),
        ("平均专注时长", stats.get("average_focus_duration", 0)),
        ("平均分心次数", stats.get("average_distraction_count", 0)),
        ("平均完成率", stats.get("completion_rate", 0))
    ]
    ...
    focus_dist = stats.get("focus_level_distribution", {})
    if focus_dist:
        lines.append("**专注级别分布**:")
    ...
    task_types = stats.get("task_type_coverage", [])
    if task_types:
        lines.append("**训练类型覆盖**:")
...
def export_to_html(result: Dict[str, Any]) -> str:
    ...
    lines.append("    <title>专注力训练报告</title>")
    lines.append("    <style>")
...
def render_progress_summary(progress_score: Dict[str, Any]) -> str:
    ...
    scores = [
        ("专注时长", progress_score.get("duration_score", 0)),
        ("专注稳定性", progress_score.get("stability_score", 0)),
        ("任务完成率", progress_score.get("completion_score", 0))
    ]
