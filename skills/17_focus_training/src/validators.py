"""Validation helpers for 专注力训练 Agent.
...
def validate_child_profile(profile: Dict[str, Any]) -> List[str]:
    ...
    if "attention_span_minutes" in profile:
        span_errors = validate_duration(profile["attention_span_minutes"], 0, 60)
        errors.extend([f"注意力时长: {e}" for e in span_errors])

    return errors


def validate_focus_activity(activity: Dict[str, Any]) -> List[str]:
    """验证专注力训练活动
    ...
    if "task_type" in activity:
        task_type_errors = validate_task_type(activity["task_type"])
        errors.extend([f"任务类型: {e}" for e in task_type_errors])

    return errors


def validate_focus_session(record: Dict[str, Any]) -> List[str]:
    """验证专注力训练记录
    ...
    timestamp_errors = validate_timestamp(record.get("start_time"), "开始时间")
    errors.extend([f"开始时间: {e}" for e in timestamp_errors])

    if "focus_duration_minutes" in record:
        duration_errors = validate_duration(record["focus_duration_minutes"], 0, 120)
        errors.extend([f"专注时长: {e}" for e in duration_errors])

    if "focus_level" in record:
        focus_level_errors = validate_focus_level(record["focus_level"])
        errors.extend([f"专注级别: {e}" for e in focus_level_errors])

    if "completion_rate" in record:
        rate_errors = validate_percentage(record["completion_rate"], "完成率")
        errors.extend([f"完成率: {e}" for e in rate_errors])
...
def validate_task_type(value: Any) -> List[str]:
    """验证任务类型
    ...
    """
    errors: List[str] = []
    valid_types = {"visual", "auditory", "motor", "cognitive", "mixed"}
    ...
    if normalized not in valid_types:
        errors.append(f"任务类型必须是以下之一: {', '.join(sorted(valid_types))}")
    return errors


def validate_focus_level(value: Any) -> List[str]:
    """验证专注级别
    ...
    """
    errors: List[str] = []
    valid_levels = {"unfocused", "distracted", "focused", "deeply_focused"}
    ...
    if normalized not in valid_levels:
        errors.append(f"专注级别必须是以下之一: {', '.join(sorted(valid_levels))}")
    return errors
...
def validate_percentage(value: Any, field_name: str = "百分比") -> List[str]:
    """验证百分比值
    ...
    try:
        pct = float(value)
        if pct < 0 or pct > 100:
            errors.append(f"{field_name}必须在 0-100 范围内")
    except (ValueError, TypeError):
...
def validate_payload_with_details(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "raw_records" in payload:
        ...
        for i, record in enumerate(records):
            record_errors = validate_focus_session(record)
            if record_errors:
                result["is_valid"] = False
...
def batch_validate_records(records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for i, record in enumerate(records):
        errors = validate_focus_session(record)
        if errors:
