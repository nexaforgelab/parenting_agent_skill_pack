"""Data models for 宝宝过敏食材记录 Agent.
...
class RoutineRecord:
    ...
    routine_type: Union[str, RoutineType]
    duration_minutes: Optional[float] = None
    quality: Optional[Union[str, SleepQuality]] = None
    notes: str = ""
    baby_response: str = ""
    caregiver: str = ""
    ...
    def __post_init__(self):
        ...
        if isinstance(self.routine_type, str):
            self.routine_type = RoutineType(self.routine_type)
        if self.quality and isinstance(self.quality, str):
            self.quality = SleepQuality(self.quality)

    def to_dict(self) -> Dict[str, Any]:
        """转换为字典格式"""
        return {
            "timestamp": self.timestamp.isoformat() if isinstance(self.timestamp, datetime) else self.timestamp,
            "routine_type": self.routine_type.value if isinstance(self.routine_type, Enum) else self.routine_type,
    ...
    def validate(self) -> List[str]:
        ...
        if not self.routine_type:
            errors.append("routine_type 不能为空")
        if self.duration_minutes and (self.duration_minutes < 0 or self.duration_minutes > 1440):
            errors.append("duration_minutes 超出正常范围(0-1440分钟)")
        return errors
...
class WeeklyStats:
    ...
    average_daily_routines: float = 0.0
    routine_type_distribution: Dict[str, int] = field(default_factory=dict)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_daily_routines": round(self.average_daily_routines, 2),
            "routine_type_distribution": self.routine_type_distribution,
            "average_sleep_hours": round(self.average_sleep_hours, 2),
            "notes": self.notes
        }
