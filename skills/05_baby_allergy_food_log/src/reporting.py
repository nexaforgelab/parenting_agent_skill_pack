"""Markdown report renderer for 宝宝过敏食材记录 Agent.
...
def render_weekly_stats_table(stats: Dict[str, Any]) -> List[str]:
    ...
    metrics = [
        ("总作息事件数", stats.get("total_routines", 0)),
        ("日均作息事件", stats.get("average_daily_routines", 0)),
        ("平均睡眠时长(小时)", stats.get("average_sleep_hours", 0))
    ]
    ...
    routine_dist = stats.get("routine_type_distribution", {})
    if routine_dist:
        lines.append("**作息类型分布**:")
        for rt, count in routine_dist.items():
            lines.append(f"- {rt}: {count}次")
        lines.append("")
...
def export_to_html(result: Dict[str, Any]) -> str:
    ...
    lines.append("    <title>宝宝作息规律培养报告</title>")
    lines.append("    <style>")
