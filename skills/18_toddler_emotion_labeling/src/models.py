...
class EmotionLabel:
    """情绪标签数据模型"""
    emotion_id: str
    emotion_type: Union[str, EmotionType]
    intensity: Union[str, EmotionIntensity] = EmotionIntensity.MODERATE
    labeler: str = "parent"
    ...
    def __post_init__(self):
        ...
        if isinstance(self.emotion_type, str):
            try:
                ...
                self.emotion_type = EmotionType.NEUTRAL
        if isinstance(self.intensity, str):
            try:
                self.intensity = EmotionIntensity(self.intensity)
            except ValueError:
                self.intensity = EmotionIntensity.MODERATE

    def to_dict(self) -> Dict[str, Any]:
        """转换为字典格式"""
        return {
            "emotion_id": self.emotion_id,
            "emotion_type": self.emotion_type.value if isinstance(self.emotion_type, Enum) else self.emotion_type,
            "intensity": self.intensity.value if isinstance(self.intensity, Enum) else self.intensity,
    ...
    def validate(self) -> List[str]:
        ...
        if not self.emotion_id:
            errors.append("emotion_id 不能为空")
        if self.confidence < 0 or self.confidence > 1:
            errors.append("confidence 必须在 0-1 范围内")
        return errors
...
class EmotionObservation:
    ...
    trigger_event: str = ""
    expression_method: Union[str, ExpressionMethod] = ExpressionMethod.MIXED
    ...
    def __post_init__(self):
        ...
        if isinstance(self.expression_method, str):
            try:
                self.expression_method = ExpressionMethod(self.expression_method)
            except ValueError:
                self.expression_method = ExpressionMethod.MIXED
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "emotion_labels": [e.to_dict() if isinstance(e, EmotionLabel) else e for e in self.emotion_labels],
            "trigger_event": self.trigger_event,
            "expression_method": self.expression_method.value if isinstance(self.expression_method, Enum) else self.expression_method,
    ...
    def validate(self) -> List[str]:
        ...
        if not self.observation_id:
            errors.append("observation_id 不能为空")
...
class WeeklyStats:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "successfully_regulated": self.successfully_regulated,
            "regulation_success_rate": round(self.regulation_success_rate, 2),
            "notes": self.notes
        }
