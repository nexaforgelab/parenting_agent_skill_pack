"""Validation helpers for 幼儿情绪识别 Agent.
...
def validate_emotion_observation(record: Dict[str, Any]) -> List[str]:
    """验证情绪观察记录
    ...
    timestamp_errors = validate_timestamp(record.get("timestamp"), "观察时间")
    errors.extend([f"观察时间: {e}" for e in timestamp_errors])

    if "emotion_type" in record:
        emotion_errors = validate_emotion_type(record["emotion_type"])
        errors.extend([f"情绪类型: {e}" for e in emotion_errors])

    if "intensity" in record:
        intensity_errors = validate_emotion_intensity(record["intensity"])
        errors.extend([f"情绪强度: {e}" for e in intensity_errors])

    if "confidence" in record and record["confidence"] is not None:
        confidence_errors = validate_confidence(record["confidence"])
        errors.extend([f"置信度: {e}" for e in confidence_errors])
...
def validate_emotion_type(value: Any) -> List[str]:
    """验证情绪类型
    ...
    """
    errors: List[str] = []
    valid_types = {"happy", "sad", "angry", "afraid", "surprised", "disgusted", "neutral", "confused"}

    if value is None or str(value).strip() == "":
        errors.append("情绪类型不能为空")
        return errors

    normalized = str(value).lower().strip()
    if normalized not in valid_types:
        errors.append(f"情绪类型必须是以下之一: {', '.join(sorted(valid_types))}")
    return errors


def validate_emotion_intensity(value: Any) -> List[str]:
    """验证情绪强度
    ...
    """
    errors: List[str] = []
    valid_intensities = {"mild", "moderate", "strong", "overwhelming"}
    ...
    if normalized not in valid_intensities:
        errors.append(f"情绪强度必须是以下之一: {', '.join(sorted(valid_intensities))}")
    return errors


def validate_confidence(value: Any) -> List[str]:
    """验证置信度
...
def validate_payload_with_details(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "raw_records" in payload:
        ...
        for i, record in enumerate(records):
            record_errors = validate_emotion_observation(record)
            if record_errors:
                result["is_valid"] = False
...
def batch_validate_records(records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for i, record in enumerate(records):
        errors = validate_emotion_observation(record)
        if errors:
