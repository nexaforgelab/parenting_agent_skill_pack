...
SKILL_ID = "preschool_english_starter"
SKILL_NAME = "幼儿英语启蒙 Agent"
RISK_NOTES = ['本 Skill 提供家庭教育和生活管理建议，不替代专业人士意见。', '涉及安全、法律、医疗或教育政策时，应提示家长二次核验。', '输出应尊重家庭差异，不用单一标准评价孩子或父母。']
...
def run(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    mastered_words = progress.get("mastered_words", 0)
    progress_stats = {
        "total_words": total_words,
        "mastered_words": mastered_words,
        "learning_words": progress.get("learning_words", 0),
        "average_mastery": progress.get("average_mastery", 0.0),
        "mastery_rate": (mastered_words / total_words * 100) if total_words > 0 else 0.0,
        "difficult_words": progress.get("difficult_words", []),
    ...
    result: Dict[str, Any] = {
        ...
        "summary": [
            ...
            f"当前英语学习进度：共学习 {progress.get('total_words', 0)} 个单词，掌握 {progress.get('mastered_words', 0)} 个。",
