"""Markdown report renderer for 幼儿数学启蒙 Agent.
...
def render_weekly_stats_table(stats: Dict[str, Any]) -> List[str]:
    ...
    metrics = [
        ("总活动数", stats.get("total_activities", 0)),
        ("日均活动数", stats.get("average_daily_activities", 0)),
        ("掌握活动数", stats.get("mastered_activities", 0))
    ]
    ...
    engagement = stats.get("engagement_scores", {})
    if engagement:
        lines.append("**参与度分布**:")
        for level, score in engagement.items():
            lines.append(f"- {level}: {score:.1%}")
        lines.append("")
...
def export_to_html(result: Dict[str, Any]) -> str:
    ...
    lines.append("    <title>幼儿数学启蒙报告</title>")
    lines.append("    <style>")
...
def render_progress_summary(progress_score: Dict[str, Any]) -> str:
    ...
    scores = [
        ("掌握程度", progress_score.get("mastery_score", 0)),
        ("参与度", progress_score.get("engagement_score", 0)),
