"""Markdown report renderer for 小学生阅读计划 Agent.
...
class ProgressVisualizer:
    ...
    def generate_progress_summary(progress: Dict[str, Any]) -> List[str]:
        """生成进度摘要"""
        summary = []
        summary.append(f"总阅读次数：{progress.get('total_sessions', 0)} 次")
        summary.append(f"阅读书籍数：{progress.get('total_books', 0)} 本")
        summary.append(f"阅读页数：{progress.get('total_pages', 0)} 页")
        summary.append(f"完成率：{progress.get('completion_rate', 0.0)*100:.1f}%")
        summary.append(f"平均投入度：{progress.get('average_engagement', 0.0):.1f}/5")
...
class SummaryGenerator:
    ...
    def generate_executive_summary(result: Dict[str, Any]) -> str:
        ...
        if progress:
            lines.append(f"**学习进度**: 完成 {progress.get('total_sessions', 0)} 次阅读")
            lines.append(f"**阅读量**: {progress.get('total_pages', 0)} 页")
...
def render_report(result: Dict[str, Any]) -> str:
    ...
    if progress:
        ...
        if progress.get("favorite_genres"):
            lines.append("")
            lines.append(f"**喜欢的类型**: {', '.join(progress['favorite_genres'])}")
...
def export_report(result: Dict[str, Any], format: str = "markdown") -> str:
    ...
    if format == "json":
        ...
        records = result.get("raw_records", [])
        columns = ["timestamp", "book_title", "author", "pages_read", "engagement_score"]
