"""Validation helpers for 小学生阅读计划 Agent.
...
def validate_learning_record(record: Dict[str, Any]) -> List[str]:
    ...
    pages_read = record.get("pages_read", 0)
    if pages_read < 0:
        errors.append("pages_read 不能为负数")

    engagement_score = record.get("engagement_score", 0.0)
    error_list = validator.range(engagement_score, 0, 5, "engagement_score")
    for err in error_list:
        errors.append(str(err))

    genre = record.get("genre", "")
    if genre:
        allowed_genres = ["文学", "科普", "历史", "童话", "绘本", "其他"]
        error_list = validator.is_in(genre, allowed_genres, "genre")
        for err in error_list:
            errors.append(str(err))
