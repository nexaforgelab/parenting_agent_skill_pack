"""Markdown report renderer for 新生儿喂养记录 Agent.
...
def render_weekly_stats_table(stats: Dict[str, Any]) -> List[str]:
    ...
    metrics = [
        ("总喂养次数", stats.get("total_feedings", 0)),
        ("日均喂养次数", stats.get("average_daily_feedings", 0)),
        ("总喂养量(ml)", stats.get("total_amount_ml", 0)),
        ("日均喂养量(ml)", stats.get("average_amount_ml", 0))
    ]
    ...
    feeding_dist = stats.get("feeding_type_distribution", {})
    if feeding_dist:
        lines.append("**喂养类型分布**:")
        for ft, count in feeding_dist.items():
            lines.append(f"- {ft}: {count}次")
        lines.append("")

    bowel_summary = stats.get("bowel_movement_summary", {})
    if bowel_summary:
        lines.append("**排便情况摘要**:")
        for bm, count in bowel_summary.items():
            lines.append(f"- {bm}: {count}次")
        lines.append("")
...
def export_to_html(result: Dict[str, Any]) -> str:
    ...
    lines.append("    <title>新生儿喂养记录报告</title>")
    lines.append("    <style>")
