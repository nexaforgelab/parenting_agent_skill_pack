"""Validation helpers for 课外班避坑 Agent.
...
def require_payload(payload: Dict[str, Any]) -> List[str]:
    ...
    if history_days is not None:
        ...
        elif history_days < 1 or history_days > 90:
...
def validate_class_info(class_info: Dict[str, Any]) -> List[str]:
    ...
    if not name or not str(name).strip():
        errors.append("课外班名称不能为空")

    institution = class_info.get("institution")
    if not institution or not str(institution).strip():
        errors.append("培训机构名称不能为空")

    tuition = class_info.get("tuition")
