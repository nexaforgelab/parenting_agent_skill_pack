"""Markdown report renderer for 小学英语单词 Agent.
...
class TableFormatter:
    ...
    @staticmethod
    def format_progress_bar(value: float, max_value: float = 100.0, width: int = 20, prefix: str = "", suffix: str = "") -> str:
        """格式化进度条"""
...
class ProgressVisualizer:
    ...
    def generate_progress_summary(progress: Dict[str, Any]) -> List[str]:
        ...
        summary.append(f"学习单词数：{progress.get('total_words', 0)} 个")
        summary.append(f"正确率：{progress.get('mastery_rate', 0.0)*100:.1f}%")
        summary.append(f"平均得分：{progress.get('average_score', 0.0):.1f}/100")
...
class SummaryGenerator:
    ...
    def generate_executive_summary(result: Dict[str, Any]) -> str:
        ...
        if progress:
            ...
            lines.append(f"**掌握率**: {progress.get('mastery_rate', 0.0)*100:.1f}%")
...
def render_report(result: Dict[str, Any]) -> str:
    ...
    if progress:
        ...
        if progress.get("weak_words"):
            lines.append("")
            lines.append(f"**易错单词**: {', '.join(progress['weak_words'][:5])}")
...
def export_report(result: Dict[str, Any], format: str = "markdown") -> str:
    ...
    if format == "json":
        ...
        records = result.get("raw_records", [])
        columns = ["timestamp", "words_practiced", "correct_count", "score", "topic"]
