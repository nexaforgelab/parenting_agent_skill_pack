"""Validation helpers for 小学英语单词 Agent.
...
def validate_learning_record(record: Dict[str, Any]) -> List[str]:
    ...
    error_list = validator.range(score, 0, 100, "score")
    for err in error_list:
        errors.append(str(err))

    words_practiced = record.get("words_practiced", 0)
    if words_practiced < 0:
        errors.append("words_practiced 不能为负数")

    practice_type = record.get("practice_type", "")
    if practice_type:
        allowed_types = ["听音辨词", "拼写测试", "单词配对", "造句练习", "阅读理解"]
        error_list = validator.is_in(practice_type, allowed_types, "practice_type")
        for err in error_list:
            errors.append(str(err))
