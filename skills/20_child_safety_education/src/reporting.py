"""Markdown report renderer for 儿童安全教育 Agent.
...
def render_weekly_stats_table(stats: Dict[str, Any]) -> List[str]:
    ...
    metrics = [
        ("总培训次数", stats.get("total_sessions", 0)),
        ("完成率", stats.get("completion_rate", 0)),
        ("平均正确率", stats.get("average_score", 0)),
        ("完成课程数", stats.get("lessons_completed", 0)),
        ("需要强化", stats.get("reinforcement_needed", 0))
    ]
...
def export_to_html(result: Dict[str, Any]) -> str:
    ...
    lines.append("    <title>儿童安全教育报告</title>")
    lines.append("    <style>")
    lines.append("        body { font-family: 'Microsoft YaHei', Arial, sans-serif; max-width: 900px; margin: 0 auto; padding: 20px; }")
    lines.append("        h1 { color: #c0392b; border-bottom: 3px solid #e74c3c; padding-bottom: 10px; }")
...
def render_progress_summary(progress_score: Dict[str, Any]) -> str:
    ...
    scores = [
        ("安全知识", progress_score.get("knowledge_score", 0)),
        ("实践能力", progress_score.get("practice_score", 0)),
        ("主题覆盖", progress_score.get("coverage_score", 0))
    ]
